...
class modules_project_member extends modules_common
{
    ...
    function updateSpecial($members, $managers, $projectId)
    {
        ...
        $_members = $this->getAll();
        if (is_array($_members) && sizeof($_members)) {
        ...
        $missingManagers = array_diff($managers, $allUserIdsInDb);
        if (is_array($missingManagers) && sizeof($missingManagers)) {
            $newData = array();
        ...
        $missingMembers = array_diff($members, $allUserIdsInDb);
        if (is_array($missingMembers) && sizeof($missingMembers)) {
            $newData = array();
...
// FIXXXME return true if update succeeded
    ...
    function isMember($projectId = null, $userId = null)
    {
        ...
        if ($projectId == null && is_array($isMemberIn) && sizeof($isMemberIn)) {
            return true;
        }
    ...
    function getManagerProjects($userId = null)
    {
        ...
        if (is_array($memberProjects) && sizeof($memberProjects)) {
