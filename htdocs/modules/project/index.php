...
if (isset($_REQUEST['tree']['add']) && is_array($_REQUEST['tree']['add'])) {
    ...
    if (empty($_REQUEST['tree']['add']['startDate'])) {
        $_REQUEST['tree']['add']['startDate'] = 0;
    }
    $_REQUEST['tree']['add']['endDate'] = $util->makeTimestamp($_REQUEST['tree']['add']['endDate']);
    if (empty($_REQUEST['tree']['add']['endDate'])) {
        $_REQUEST['tree']['add']['endDate'] = 0;
    }
...
if (isset($_REQUEST['tree']['update']) && is_array($_REQUEST['tree']['update'])) {
    ...
    if (empty($_REQUEST['tree']['update']['startDate'])) {
        $_REQUEST['tree']['update']['startDate'] = 0;
    }
    $_REQUEST['tree']['update']['endDate'] = $util->makeTimestamp($_REQUEST['tree']['update']['endDate']);
    if (empty($_REQUEST['tree']['update']['endDate'])) {
        $_REQUEST['tree']['update']['endDate'] = 0;
    }
    // write null in the table, so there is really no value it wont confuse the user
    // and we can still use 0
...
if (!empty($_REQUEST['removeId'])) {
    // with this new call I delete all times and project
    // memberships for the given project (and subprojects).
    $projectTree->RemoveProject(intval($_REQUEST['removeId']));
    ...
    $newdata['remove']['id'] = intval($_REQUEST['removeId']);
    $newdata['action']['remove'] ='remove';
    // we also delete subprojects
    $projectTree->setRemoveRecursively(true);
} else {
...
if (!empty($_REQUEST['id'])) {
    $editFolder = $projectTree->getElement(intval($_REQUEST['id']));
}
