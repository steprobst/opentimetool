...
$_times = $time->getDay($show['dateFrom'], $show['dateUntil']);
if (is_array($_times) && !empty($_times)) {
    $_lastDate = 0;
    $times = array();
...
if (isset($show['projectTree_ids']) && is_array($show['projectTree_ids']) &&
        sizeof($show['projectTree_ids']) == 1) {
...
if (isset($_REQUEST['action_toHTML']) || isset($_REQUEST['action_toPDF'])) {
    ...
    if (!@is_dir($config->exportDir)) {
        if (!mkdir($config->exportDir, 0775)) {
