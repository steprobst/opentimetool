...
// +----------------------------------------------------------------------+
//
//  $Id: Memory.php 123 2019-09-03 15:03:36Z munix9 $
//
// Anton kejr : Bugfix in _getElement 	25.08.2006
...
class Tree_Memory extends Tree_Common
{
    ...
    */
    function __construct( $type , $dsn='' , $options=array() )
    {
        Tree_Options::__construct($options); // set the options for $this
...
//FIXXME this is not really cool, maybe overwrite the *Option* methods!!!
    ...
    */
    function switchDataSource( $type , $dsn='' , $options=array() )
    {
        ...
        // AK : is that really working or at least useful ???? index dummy isn't defined !!
        self::__construct( $type , @$GLOBALS['dummy'] , $options );
...
//print_r($this->children);
        ...
        if( is_array($this->children[0]) && sizeof($this->children[0]) )
...
// TODO PEAR-ize error
    ...
    */
    function walk( $walkFunction , $id=0 , $returnType='string')
    {
        ...
        $this->walkReturn = array();                // a new walk starts, unset the return value
    ...
    function _walk( $walkFunction , &$curLevel , $returnType )
    {
        if (isset($curLevel) && is_array($curLevel)) {
            foreach ($curLevel as $key=>$value) {
                $ret = call_user_func_array( $walkFunction , array(&$this->data[$key]) );
                switch ($returnType) {
    ...
    */
    function addNode( $node )
    {
        ...
        foreach( $node as $aNode )
        {
            ...
            {
                if( is_array($newNode) && sizeof($newNode) )              // if there are children, set their parentId, so they kknow where they belong in the tree
...
// since a while is always a little risky
    ...
    */
    ...
    {
    	// AK : eliminate a notice from php :
    	// Only variable references should be returned by reference in 
    	// /srv/www/htdocs/openTimetool/includes/Tree/Memory.php on line 824	
    	$nothing = NULL;
		...
		// AK See above ! was 'return NULL'
        return $nothing;    // we should not return false, since that might be a value of the element that is requested
    } // end of function
        ...
        if (sizeof($node)) {
            ...
            foreach ($nodes as $aNode) {
                ...
                foreach ($keys as $aKey) {
                    if (!is_array($key)) {
                        $val = isset($aNode[$aKey]) ? $aNode[$aKey] : '&nbsp;'; // AK : isset
                        print "<td>$val</td>";
                    }
                }
                print "</tr>";
