...
// +----------------------------------------------------------------------+
//
// $Id: Query.php 123 2019-09-03 15:03:36Z munix9 $
//
// $Id
//
// 1.4.1  2006/08/28		AK		
// php4 -> php5 : array merge (line 1316) needs 2 arrays. in php4 there was an implicite cast
//				  php5 needs an explicite one !
//
...
// FIXXME what shall we do here?
    ...
    */
    function get( $id , $column='' )
    {
        ...
        $exec = $this->execute($queryString,$getMethod);
        return $this->returnResult( $exec );
    }
...
*/
        ...
        $res = $this->execute($queryString,$method);
...
// then the users dont have to write that explicitly and can use the same name as in the setOrder i.e. setOrder('name,_net_name,_netPrefix_prefix');
    ...
    */
    ...
    {                         
        ...
        $string = $this->_db->quote('%'.str_replace(' ','%',mb_strtolower($string)).'%');
...
// FIXXXME use oci8 implementation of newer PEAR::DB-version
    ...
    */
    function _buildFrom()
    {
        ...
        {
        	// AK : I found we got here in several times when selecting "Team" on the left.
        	// The first 2 times $join is more or less empty and doesn't contain key 'default'
        	
            // handle the standard join thingy
            if ( isset($join['default']) ) {			  // AK : to avoid php notices
            	if(isset($join['default']['table']))	  // AK : to avoid php notices
...
//print "$what ... ";print_r( $res );print "<br>";
            ...
            if (in_array('',$selectAllFromTables)) { // was there a '*' ?
                ...
                // AK : array cast introduced in array_merge : php4 vs php5 : (array)$this->table)
                $tables = $this->getJoin() ?
...
// doesnt work on oracle, add a letter before it!!!
    ...
    */
    ...
    {
    	$keyValue = ''; // AK : just avoid notices 
