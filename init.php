 ...
 */
if (!isset($session->temp->dbUpgrade)) {
    require_once $config->classPath . '/dbUpgrade.php';
    $dbUpgrade = new dbUpgrade($config, $db);
    $dbUpRes = $dbUpgrade->run();
    @$session->temp->dbUpgrade = true;
    if ($dbUpRes !== 0) {
        header('Location: ' . $config->home);
        exit;
    }
...
if (isset($_REQUEST['isDesktop'])) {
    $util->isDesktop((bool)$_REQUEST['isDesktop']);
}
...
$properties = array(
    ...
    'modules/system'         => array('pageHeader'=>'TSM · openTimetool System Menu'),
);

if (!$util->isDesktop()) {
    $properties['modules/time/today']['pageHeader'] = 'Mobile-Log';
}
